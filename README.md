...
finding the nearest (to the screen) z-coordinate in each patch that will be
represented by a
...
version 0.15.0, zig 0.15 is currently the only supported release). Also, it has
only been tested on linux...though it should work anywhere that is relatively
posix compliant, best of luck on that.

```{bash}
git clone https://github.com/Braden-Griebel/zigonut.git
cd zigonut
zig build -Doptimize=ReleaseFast # or ReleaseSmall, ReleaseSafe
./zig-out/bin/zigonut
```

## Licensing

The code in this repository is licensed under an MIT license. Zigonut makes use
of [zig-clap](https://github.com/Hejsil/zig-clap), which is also licensed under
an MIT license.
